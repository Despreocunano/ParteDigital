    ...
    <section className={`py-24 px-4 ${className}`}>
      <div className="max-w-6xl mx-auto text-center">
        <div className="w-16 h-16 bg-[#B76E79]/10 rounded-full flex items-center justify-center mx-auto mb-6">
          <Instagram className="w-8 h-8 text-[#B76E79]" />
        </div>
        
        <h2 className="text-3xl md:text-4xl font-serif mb-8 text-[#B76E79]">Comparte tus Fotos</h2>
        
        {instagramHashtag && (
          <p className="text-2xl font-light text-[#B76E79]/80 mb-12">
            #{instagramHashtag}
          </p>
        )}
        ...
        {instagramPosts?.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {instagramPosts.map((post) => (
              <div 
                key={post.id}
                className="aspect-square overflow-hidden rounded-xl border border-[#B76E79]/10"
              >
                <img
                  src={post.imageUrl}
